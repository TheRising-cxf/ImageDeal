...
#include <cmath>
#include <string.h>
int F_GrindskinByGaussFilter(unsigned char* srcData, int width, int height, int channels, int skinMode, int ratio)
{
	int ret = 0;
	...
	ret = F_SmartBlur(smoothData, width, height, channels, smoothRadius, smoothThreshold);
	if (skinMode == 0)
	{
		ret = F_SkinDetectionYCgCr(skinProbability, width, height, channels);
		maskSmoothRadius = 6;
	}
	else
	{
		ret = F_SkinProbability(skinProbability, width, height, channels);
	}
	ret = F_FastGaussFilter(skinProbability, width, height, channels, maskSmoothRadius);
...
int F_Softskin_ChannelMethod(unsigned char* srcData, int width, int height, int channels, unsigned char* lightMap, int ratio)
{
	int ret = 0;
	unsigned char* greenData = (unsigned char*)malloc(sizeof(unsigned char)* width * channels * height);
	unsigned char* gaussData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* curveData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* skinData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* pSrc = srcData;
	...
	memcpy(gaussData, greenData, sizeof(unsigned char) * width * height * channels);
	memcpy(curveData, srcData, sizeof(unsigned char) * width * height * channels);
	ret = F_Filter512(curveData, width, height, channels, lightMap);
	float hpRadius = 10.0f * width * height / (594 * 677);
	ret = F_FastGaussFilter(gaussData, width, height, channels, hpRadius);
	pSrc = srcData;
	pGreen = greenData;
	...
	memcpy(skinData, greenData, sizeof(unsigned char)* width * height * channels);
	int maskSmoothRadius = 3 * width * height / (594 * 677);
	ret = F_SkinProbability(skinData, width, height, channels);
	ret = F_FastGaussFilter(skinData, width, height, channels, maskSmoothRadius);
	pGauss = skinData;
	pSrc = srcData;
	pGreen = greenData;
	int k = ratio * 128 / 100;
...
int F_Softskin_DetailsAddingMethod(unsigned char* srcData, int width, int height, int channels, int ratio, float K)
{
	int ret = 0;
	int len = sizeof(unsigned char) * width * height * channels;
	...
	int std_fine = 5;
	int std_coarse = 10;
	F_SmartBlur(fineSmoothData, width, height, channels, std_fine, 30);
	F_SmartBlur(coarseSmoothData, width, height, channels, std_coarse, 30);
...
int F_Softskin_HP(unsigned char* srcData, int width, int height, int channels, int textureRatio, int ratio)
{
	int ret = 0;
	int length = width * height * channels;
	unsigned char* smoothData = (unsigned char*)malloc(sizeof(unsigned char) * length);
	...
	int smoothThreshold = 38;
	int maskSmoothRadius = 3;
	ret = F_SurfaceBlur(smoothData, width, height, channels, smoothRadius, smoothThreshold);
	...
	ret = F_FastGaussFilter(hpData, width, height, channels, hpRadius);
	pSmooth = smoothData;
	pHP = hpData;
	pSrc = srcData;
...
int F_Softskin_A(unsigned char* srcData, int width, int height, int channels, unsigned char* lightMap, int ratio)
{
	int ret = 0;
	unsigned char* greenData = (unsigned char*)malloc(sizeof(unsigned char)* width * channels * height);
	unsigned char* gaussData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* curveData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* skinData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* smoothData = (unsigned char*)malloc(sizeof(unsigned char) * width * channels * height);
	unsigned char* pSrc = srcData;
	unsigned char* pGreen = greenData;
	for (int j = 0; j < height; j++)
	{
	...
	memcpy(gaussData, greenData, sizeof(unsigned char) * height * channels);
	memcpy(curveData, srcData, sizeof(unsigned char) * height * channels);
	ret = F_Filter512(curveData, width, height, channels, lightMap);
	...
	int k = ratio * 128 / 100;
	memcpy(smoothData, greenData, sizeof(unsigned char) * channels * height);
	int smoothRadius = 6;
	int smoothThreshold = 38;
	ret = F_SmartBlur(smoothData, width, height, channels, smoothRadius, smoothThreshold);
	memcpy(skinData, smoothData, sizeof(unsigned char) * height * channels);
	int maskSmoothRadius = 3;
	ret = F_SkinProbability(skinData, width, height, channels);
	ret = F_FastGaussFilter(skinData, width, height, channels, maskSmoothRadius);
	pGauss = skinData;
...
int F_Softskin_MixMethod(unsigned char* srcData, int width, int height, int channels, unsigned char* lightMap, int textureRatio, int ratio)
{
	int ret = 0;
	int length = width * height * channels;
	unsigned char* smoothData = (unsigned char*)malloc(sizeof(unsigned char) * length);
	...
	int smoothThreshold = 38;
	int maskSmoothRadius = 3;
	ret = F_Softskin_A(smoothData, width, height, channels, lightMap, 95);
	memcpy(skinPDF, smoothData, sizeof(unsigned char) * length);
	ret = F_SkinProbability(skinPDF, width, height, channels);
	unsigned char* pSrc = srcData;
	unsigned char* pSkin = skinPDF;
